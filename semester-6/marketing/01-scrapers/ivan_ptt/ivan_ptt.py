...
def ptt_crawl(Board_Name, start, page):
    ...
    for summary in latest_page: # 只要抓最新的頁面
        ...
        try:
            ...
            board.append(article.board)
            aid.append(article.aid)
            author.append(article.author)
            ip.append(article.ip)
            pushes_All.append(article.pushes.count['all'])
            pushes_Content.append(article.pushes.simple_expression)
            pushes_Like.append(article.pushes.count['like'])
            pushes_boo.append(article.pushes.count['boo'])
            pushes_neutral.append(article.pushes.count['neutral'])
