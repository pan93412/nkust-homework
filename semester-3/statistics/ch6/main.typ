...
#question("ex6.7")[
  假設一公車，在早上 7:00~7:30 之間到達某站牌的時間為均勻分配。有一天，阿輝剛好 7:00 到達該站牌，試問：

  + 阿輝等待的時間超過10分鐘的機率
  + 阿輝等待時間的 $E[X]$ 和 $V(X)$
][
  $7:00~7:30$ 的區間，令左界為 $a$，右界為 $b$。已知均勻分配的機率密度函數 (@def-6.3.1)，故以此例來說：
  ...
  *Q1*
  ...
  $
  P(Y>=10) &= integral^30_10 1/30 dif y \
           &= 1/30 y |_10^30 \
           &= 1/30 (30 - 10) \
           &= 2/3
  $
...
類似先前提及的離散型隨機變數：卜瓦松隨機變數（某一單位區間 $t$ 發生之次數 $lambda$），「指數分配」是連續型隨機變數，表示兩兩之間所間隔的時間（以隨機變數 $Y$ 表示）的機率分配。

#blk[
  *定義 6.4.1*: 連續型隨機變數 $Y$，若其機率密度函數為：

  $
  f(y) = cases(
    lambda e^(-lambda y) "if" y>=0\; lambda > 0,
    0 "otherwise"
  )
  $
...
#blk[
  *定理 6.5*: 一個指數隨機變數 $Y$，若其參數為 $lambda$，則：

  - 期望值：$mu = E[Y] = 1/lambda$
  - 變異數：$sigma^2 = V(Y) = 1/lambda^2$
]
