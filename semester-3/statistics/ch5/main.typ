...
一般而言，當採抽取不放回時，$(n/N)>=0.05$ 時仍用超幾何分配求算，而 $(n/N)<0.05$ 時，二項分配會是超幾何分配良好的近似值，且比較容易求算。

#question("ex5.19")[
  飛而普公司某一月份生產燈泡 $N$ 個，其中有 $5%$ 為不良品。今天從這批產品中隨機一次抽取 5 個（抽取不放回），試問以下兩個情形中，抽取五個燈泡 #comment($n=5$) 中不良品個數的機率為何？

  + $N=40$
  + $N=1000$
][
  + $N=40, n=5, r=40 times 0.05 = 2, n/N = 5/40 = 0.125 > 0.05$, 以超幾何分配求算。令 $Y$ 表示「隨機抽取 5 個燈泡中的不良品個數），則有一個不良品的機率為：
    $
    P(Y=1) = H(1;40,5,2) = C^2_1 C^(40-2)_(5-1) \/ C^40_5 = 0.2244
    $

  + $N=1000, n=5, r=1000 times 0.05=50, n/N = 5/1000 = 0.005 < 0.05$，以二項分配求算。令 $X$ 表示「隨機抽取 5 個燈泡中的不良品個數」，則有一個不良品的機率為：
    $
    P(X=1) = b(1;5,0.05) = C^5_1 (0.05)^1 (0.95)^4 = 0.2036
    $
]
...
其中 $r$ 為欲達到「成功」的次數，$p$ 為「成功」機率，$q=1-p$ 為「失敗」機率。
...
#question("ex5.21")[
  在一個不透明桶子中，有 15 顆白球、5 顆黑球。今天依序從桶中抽取球，採抽取放回方式。令 *$X$* 表示「直到抽取到第 3 顆黑球為止，所需抽取的總次數」，求：

  + $X$ 的機率分配
  + 直到第5次抽取時，才抽取到第 3 顆黑球的機率
  + 期望值 $E[Y]$ 和變異數 $V(Y)$
][
  *Q1*

  $
  P(X=x) = C^(x-1)_2 (5/20)^3 (15/20)^(x-3) &, space x=3,4,5,...
  $

  *Q2*

  $
  P(X = 5) = C^4_2 (5/20)^3 (15/20)^2 approx 0.0527
  $
  ...
  *Q3*
  ...
  r=3&, p=5/20 \
  E[Y] &= 3/(5/20) = 12 \
  V(Y) &= (3 times (15/20))/(5/20)^2 = 36
  $
]

== 卜瓦松分配

某一*單位區間內*（可能是時間、距離、空間），觀察某個特定事件發生。
...
+ 一小時內，某一路口發生的車禍次數。
+ 台灣在一年內受到颱風侵襲的次數。
+ 初春時，在面積為 $100$ 平方公尺的花園內，觀察以盛開的花朵數量。

=== 特性

+ 在一個單位區間（如時間、面積）內，此特定稀少事件發生平均次數 $lambda$ 通常是已知且固定。
+ 在單位區間內事件發生的平均次數 $lambda$ 通常與區間大小 $t$ 成正比，因此在 $t$ 個單位時間內，事件發生的平均次數為 $lambda t$（如一個便利超商每小時假設平均有 10 名顧客，則每 2 小時應平均有 20 名顧客；每 $t$ 小時應平均有 $10t$ 個顧客）。
+ 不管這個事件在這個區間的哪個點發生，發生的機率必定都相同
+ 假設這個實驗可以將單位區間分割為極小的區間，每一個區間至少可以發生一件此特定事件（成功），或者是沒有這個事件發生（失敗）。換句話說，每一個小區間，可能發生的結果只有兩類：成功、失敗。
+ 在兩個互斥區間 (disjoint intervals) 或沒有重疊的區間，事件發生的個數相互獨立。

=== 機率分配函數

Definition 5.6.1. 卜瓦松隨機變數 $Y$，機率分配稱作卜瓦松分配 (Poisson Distribution)，為：

$
P(Y=y) = lambda^y/(y!) e^(-lambda) &, space y=0,1,2,...
$

其中 $lambda$ 為某事件在單位區間（如單位時間、單位面積）內發生的平均次數，已知 $e=2.71828$ 為自然數。
...
#question("ex5.23")[
  台北市 *每一個小時* 內，*平均發生一次搶案* #comment[一段時間內發生的次數，$lambda=1$]。假設 $Y$ 符合卜瓦松分配，試問：
  ...
  *Q1*
  ...
  一小時內，完全沒有發生搶案的機率：$Y=0$

  $
  P(Y=0) = 1^0/0! e^(-1) = 0.3679
  $

  *Q2*

  一小時內，發生搶案超過兩次的機率：$Y>2$

  $
  P(Y>2) &= 1-P(Y=0)-P(Y=1)-P(Y=2) \
         &= 1-((1^0e^(-1))/0!) - ((1^1e^(-1))/1!) - ((1^2e^(-1))/2!) \
          &= 1-0.3679-0.3679-0.1839 = 0.0803
  $

  其中 $e^-1 = 1/e$, $e=2.71828$

  *Q3*

  兩小時內，恰巧只發生一次搶案的機率：$lambda_b=2 times 1 = 2$，$Y=1$，且令 $P(X=x)$ 使用 $lambda_b$:

  $
  P(X=x) = lambda_b^y/(y!) e^((-lambda_b))
  $
...
當 $n->oo$ 時，二項分配趨近於卜瓦松分配。

$
lim_(n->oo) C^n_y p^y (1-p)^(n-y) = lambda^y/(y!) e^(-lambda) &, space "where" lambda = n p
$

見 p5-30, ex5.24. (下週/* wip: key first */)

#question("ex5.24")[
  假設一個機械工廠出產的產品中，有 $1%$ 為不良品。今天向這個工廠訂購 $100$ 件產品，試問這 $100$ 件產品中，至少有一件不良品的機率是多少？
][
  考慮到產品只有好、壞，故為二項分配。令 $X$ 代表 100 件產品中不良品的個數：$X~b(x;n=100;p=0.01)$:

  $
  P(X>=1) &= 1-P(X=0) \
          &= 1-(C^100_0 (0.01)^0 (0.99)^100) \
          &= 1-0.366 = 0.634
  $

  若使用上述結論：當 $n$ 很大，$p$ 很小，且 $n p=100 times 0.01 = 1<7$，則二項分配趨近於卜瓦松分配：

  $
  P(X>=1) &= 1-P(X=0) \
          &= 1-(lambda^0/(0!) e^(-lambda)) \
          &= 1-(1^0/(0!) e^(-1)) \
          &= 1-0.367 = 0.633
  $
]
