...
#formula[
  *樣本變異數*（快速解法）
  ...
  $
  s^2 &= 1/n sum(x-overline(x))^2 \
      &= 1/n (x^2-2 x overline(x) + overline(x)^2) \
      &= 1/n sum x^2 - 2 overline(x) times 1/n sum x + 1/n sum overline(x)^2
  $

  And we can know that $1/n sum x$ is $overline(x)$, while $1/n sum overline(x)^2$ is $n overline(x)^2$.

  $
  s^2 &= 1/n sum x^2 - 2 overline(x) times 1/n sum x + 1/n sum overline(x)^2 \
      &= 1/n sum x^2 - 2 overline(x) times overline(x) + 1/n times n overline(x) \
      &= 1/n sum x^2 - 2 overline(x)^2 + overline(x)^2 \
      &= 1/n sum x^2 - overline(x)^2 \
      &= 1/n (sum x_i - n overline(x)^2)
  $

  $
  &s^2 = 1/n (sum x^2 - n overline(x)^2) \
  ==>^"同乘以 n" &n s^2 = sum x^2 - n overline(x)^2 \
  ==>^"移項" &n s^2 + n overline(x)^2 = sum x^2 \
  ==> &n (s^2 + overline(x)^2) = sum x^2
  $
...
#question("ex3.19", <ex3.19>, ("變異係數",))[
  某間日光燈製造商有兩類日光燈 A、B，這兩種產品的 *樣本*
  平均壽命分別為 $overline(x)_A=3800$ 小時，以及 $overline(x)_B=4200$ 小時，而樣本標準差分別為 $s_A=280$ 小時；$s_B=360$ 小時，請問這兩種日光燈何者有較大的 (a) 樣本標準差，以及 (b) 樣本變異係數？
][
  整理題目，可得：
  $
  overline(x)_A &= 3800 \
...
- 算法
  ...
  + 若 $k/100 times n$ 不為整數，則 $P_k$ 為 $k/100 times n$ 下一個整數位置的數值。(參考 #link(<ex3.20>)[ex3.20]).

#question("Example 3.20", <ex3.20>, ("百分位數",))[
  50位同學性向測驗的成績，如 @ex3.20-f1 所示：

  #figure(caption: "50位同學性向測驗的成績")[
    #table(
      columns: (1fr,)*10,
      stroke: none,
      [22], [25], [28], [31], [34], [35],
  ...
  平均數 $overline(x)=60.36$，標準差 $s=18.61$，
  試著求出第 35 百分位數 $P_35$ 和第 50 百分位數
  $P_50$
][
...
#question("Example 3.22", <ex3.22>, ("四分位數",))[
  承 #link(<ex3.20>)[Example 3.20]，50 位學生性向測驗的成績如 @ex3.22-f1 所示，試著求出第一四分位數 $Q_1$、第二四分位數 $Q_2$、第三四分位數 $Q_3$。
...
#question("Example 3.24", <ex3.24>, ("Z值",))[
  ...
  *每小時生產量*
  ...
  經過更深入的調查，這位技工每小時的生產量 $(x_i)$ 是 $42$ 個單位，而所有 215 位技工每小時的生產量之平均值和標準差分別為 $28$ 和 $4$ 個單位，故可得這位技工生產量為 $42$ 單位的 $Z$ 值為：
...
#question("Example 3.25", <ex3.25>, ("偏態係數",))[
  承 #link(<ex3.20>)[Example 3.20]，50 位學生性向測驗的成績如 @ex3.25-f1 所示。試著求出50位學生性向測驗成績的 Bowley 偏態係數 $"SK"_B$ 和皮爾生偏態係數 $"SK"_P$。
  ...
  *Bowley 偏態係數*
  ...
  #link(<ex3.22>)[Example 3.22] 中可得知 $Q_1=46$、$Q_2=62$、$Q_3=75$。

  $
  "SK"_B  &= ((Q_3-Q_2)-(Q_2-Q_1))/(Q_3-Q_1) \
          &= (Q_3+Q_1-2Q_2)/(Q_3-Q_1) \
          &= (75+46-2 times 62)/(75-46) \
          &approx -0.103
  $
  ...
  *皮爾生偏態係數*
  ...
  根據原本的題幹，可以知道：平均數 $overline(x)=60.36$，標準差 $s=18.61$，中位數 $M_e=62$。

  $
  "SK"_P &= (3(overline(x)-M_e))/s \
         &= (3(60.36 - 62))/18.61 \
         &approx -0.26
  $

  由於 $"SK"_P<0$，可得知其為近似左偏。
]
...
- $"COV"(x, y) > 0$ $=>$ 正相關。
- $"COV"(x, y) < 0$ $=>$ 負相關。
- $"COV"(x, y) = 0$ $=>$ 無線性相關或曲線相關。

#question("Example 3.31", <ex3.31>, ("樣本共變異數",))[
  @ex3.31-f1 為美日公司在2012年1月到8月投入的廣告支出與獲益，請求出它的樣本共變異數 $"COV"(x, y)$。

  #figure(caption: "美日公司在2012年1月到8月投入的廣告支出與獲益")[
    #table(
      columns: (1fr, 1fr, 1fr,),
      [*月份*], [*廣告支出* $x_i$ (萬元)], [*獲益* $y_i$ (萬元)],
      [1], [4], [40],
      [2], [4], [50],
      [3], [6], [50],
      [4], [5], [60],
...
- 意義：假設比較台幣和美金……
  - 設 $x$ 為年齡，$y_1$ 為財產 (TWD)，$y_2$ 為財產 (USD)。
  - $"COV"(x, y_1)$ 始終大於 $"COV"(x, y_2)$，因為 $"TWD":"USD" = 30:1$.
  - 所以我們需要 *相關係數* 來去除「單位」，將數據回到同一個線上。
- 用途：衡量兩個變數 $x$ 與 $y$ 之間線性相關程度的強度與方向。
  - *沒有單位。*
...
#formula[
  *母體相關係數*
  ...
  $
  rho = "COV"(x, y)/(sigma_x sigma_y) = sigma_(x y) / (sigma_x sigma_y)
  $
]
...
#formula[
  *樣本相關係數*
  ...
  $
  r = "COV"(x, y)/(s_x s_y) = s_(x y) / (s_x s_y)
  $
]

#question("Example 3.31 (#2)", <ex3.31-2>, ("相關係數",))[
  同 #link(<ex3.31>)[Example 3.31]，請求出它的總標準差 $s$ 以及樣本相關係數 $r$。
][
  $
  ...
  #table(
    ...
    [*月份*], [*$x_i$*], [*$y_i$*], [*$x_i y_i$*], [*$x_i^2$*], [*$y_i^2$*], [*$s_x$*], [*$s_y$*],
    [1], [4], [40], [160], [16], [1600], [], [],
    [2], [4], [50], [200], [16], [2500], [], [],
    [3], [6], [50], [300], [36], [2500], [], [],
    [4], [5], [60], [300], [25], [3600], [], [],
    ...
    [7], [2], [40], [80], [4], [1600], [], [],
    [8], [3], [30], [90], [9], [900], [], [],
    [*sum*], [*33*], [*370*], [*1610*], [*151*], [*17900*], [*1.458* #footnote($sqrt((151-(33^2\/8))/(8-1))$)], [*10.606* #footnote($sqrt((17900-(370^2\/8))/(8-1))$)],
  )
...
=== 判斷
  ...
  + 強度，若 $r = [1, 1]$
    + $abs(r)<0.1$ 則無線性相關
    + $0.1<=abs(r)<0.3$ 則有弱度線性相關
    + $0.3<=abs(r)<0.5$ 則有中度線性相關
    + $0.5<=abs(r)<0.7$ 則有高度線性相關
  + 方向
    + $r=1$ 則完全呈正線性關係
    + $r=-1$ 則完全呈負線性關係
    + $r=0$ 則無線性關係或其他非線性關係（如曲線關係）

=== ⭐ 結論

共變異數與相關係數之間的關係：

- 若 $"COV"(x, y) > 0$，則 $r>0$，代表兩個變數 $x$, $y$ 呈正線性相關。
- 若 $"COV"(x, y) < 0$，則 $r<0$，代表兩個變數 $x$, $y$ 呈負線性相關。
- 若 $"COV"(x, y) = 0$，則 $r=0$，代表兩個變數 $x$, $y$ 無線性相關。

= 其他指標決定數值分佈

- 可判斷是否有 #highlight[*離群值*]
- 方法
  - 利用「謝比雪夫不等式」
  - 利用經驗法則
  - 盒鬚圖
...
== 謝比雪夫不等式 (Chebyshev Inequality)

- 不論數據呈現何種分佈，至少有 $1-1/k^2 times 100%$ 的數據，會落在 $mu plus.minus k sigma$（樣本則為 $overline(x) plus.minus k s$）範圍內。
- *此法僅適用於 #highlight[連續型資料]，且資料可以是任何分佈方式。*

=== 範例

- 令 $k=1$，則至少有 $1-(1/1^2) times 100% = 0%$ 的資料落在 $mu plus.minus 1 sigma$ 的範圍內。
- 令 $k=2$，則至少有 $1-(1/2^2) times 100% = 75%$ 的資料落在 $mu plus.minus 2 sigma$ 的範圍內。
- 令 $k=3$，則至少有 $1-(1/3^2) times 100% = 89%$ 的資料落在 $mu plus.minus 3 sigma$ 的範圍內。
- 令 $k=4$，則至少有 $1-(1/4^2) times 100% = 94%$ 的資料落在 $mu plus.minus 4 sigma$ 的範圍內。
- 令 $k=5$，則至少有 $1-(1/5^2) times 100% = 96%$ 的資料落在 $mu plus.minus 5 sigma$ 的範圍內。

甚至可以是小數：

- 令 $k=1.5$，則至少有 $1-(1/1.5^2) times 100% = 56%$ 的資料落在 $mu plus.minus 1.5 sigma$ 的範圍內。

⭐ *若不知道數據是否為常態分佈，則可使用 #underline[4倍] 或 #underline[5倍] 標準差來篩選 #underline[離群值]。* ($therefore$ $94%$ 或 $96%$ 的資料是落在這個範圍。)

#question("Example 3.27", <ex3.27>, ("謝比雪夫不等式",))[
  有一個橡皮篩工廠，生產了一堆圓柱形橡皮篩，半徑的平均數為 1.02 公分、標準差為 0.02 公分。請利用謝比雪夫不等式，估計這堆橡皮篩半徑介於 0.98 公分到 1.06 公分之間所佔的機率或比例至少為多少？
][
  整理題幹：

  - 樣本平均數 $overline(x) = 1.2$
  - 樣本標準差 $s = 0.02$
  - 範圍 (0.98, 1.06)
  ...
  $
  (1.06-0.98) / 2 = 0.04 \
  0.04 / 0.02 = 0.02 space (k = 2)
  $

  故：

  至少有 $1-(1/k^2)=1-(1/2^2)=75%$ 的數據，
  落在 $(0.98, 1.06)$ 的範圍內。
  $(1.02 - 0.02 times 2, 1.02 + 2 times 0.02) = (0.98, 1.06)$.
]

#question("Teacher-application a.", <teacher-application-a>, ("謝比雪夫不等式",))[
  假設經過高速公路的大貨車，平均重量約為 12.5 噸、標準差約為 2.2 噸，試著使用謝比雪夫不等式描述國道大貨車之重量資料。

  - 平均重量 $overline(x) = 12.5$
  - 標準差 $s = 2.2$
][
  抓出離群值：

  - 令 $k=1$，則至少有 $1-(1/1^2) times 100% = 0%$ 的資料落在 $(12.5-1 dot 2.2, 12.5+1 dot 2.2) = (10.3, 14.7)$ 的範圍內。
  - 令 $k=2$，則至少有 $1-(1/2^2) times 100% = 75%$ 的資料落在 $(12.5-2 dot 2.2, 12.5+2 dot 2.2) = (8.1, 16.9)$ 的範圍內。
  - 令 $k=3$，則至少有 $1-(1/3^2) times 100% = 89%$ 的資料落在 $(12.5-3 dot 2.2, 12.5+3 dot 2.2) = (5.9, 19.1)$ 的範圍內。
  - 令 $k=4$，則至少有 $1-(1/4^2) times 100% = 94%$ 的資料落在 $(12.5-4 dot 2.2, 12.5+4 dot 2.2) = (3.7, 21.3)$ 的範圍內。
  - 令 $k=5$，則至少有 $1-(1/5^2) times 100% = 96%$ 的資料落在 $(12.5-4 dot 2.2, 12.5+4 dot 2.2) = (1.5, bold(23.5))$ 的範圍內。

  其中 $23.5$ 代表高速公路要設計「限制大貨車重量不得超過 23.5 噸。」而使 $96%$ 的大貨車可以通過。
]

== 經驗法則 (Empirical Rule)

- 又稱 68% – 95% – 99.7% 法則。
- *此法只適用於連續型資料，且資料必須呈現 #highlight[鐘型分佈（常態分配）]。*
  - 常態分配之判別方式
    + 偏態和峰度 (見 @形狀之測量數-值) 靠近 0 時（用軟體系統計算）
    + $"中位數" = "平均數" = "眾數"$
- 定義：
  - 若數據資料呈現 #highlight[鐘型分佈]，則
    - 約有 $68%$ 的數據若在 $mu plus.minus sigma$ 範圍內 ($overline(x) plus.minus s$)。
    - 約有 $95%$ 的數據若在 $mu plus.minus 2 sigma$ 範圍內 ($overline(x) plus.minus 2s$)。
    - 約有 $99.7%$ 的數據若在 $mu plus.minus 3 sigma$ 範圍內 ($overline(x) plus.minus 3s$)。
  - *如何利用經驗法則找出離群值？*#highlight[若資料落在 $mu plus.minus 3 sigma$ 範圍外]，該筆數據稱為離群值。
- 範例
  - 假如 $overline(x)=70 "(分)"$, $s=5 "(分)"$
    - 大約有 68% 的學生成績落在 $(65, 75)$.
    - 大約有 95% 的學生成績落在 $(60, 80)$.
    - 大約有 99.7% 的學生成績落在 $(55, 85)$.
    - 若比 55 分小，或比 85 大，則為離群值。

#question("Example 3.27 (#2)", <ex3.27-2>, ("謝比雪夫不等式", "經驗法則",))[
  承 #link(<ex3.20>)[Example 3.20]，50 位學生性向測驗的成績如 @ex3.27-2-f1 所示。試著使用謝比雪夫不等式和經驗法則來估計觀察值落在其平均數的正負 $k$、$k=1,2,3$ 倍標準差的區間內之機率或比例。
...
- 定義：展示資料分佈的一種圖形，此圖形標示出資料的 *集中趨勢*、*分散或變異程度*、*偏態*、*最小值*、*最大值*。
- 此圖可用於 *任何資料分佈*，且資料可為 *連續或離散型資料*。

#image("box-and-whisker-plot.png")

#image("box-and-whisker-plot-with-outliner.png")

- $M_e$ ($Q_2$) 不一定會處於正中央。
- $Q_1 = P_25$, $Q_2 = P_50 = M_e$, $Q_3 = P_75$.
- *最小值和最大值不是離群值。*離群值應當另外標示。

// wip: 繪製 usage

=== 使用方式

如何利用盒鬚圖辨認 #highlight[離群值]？

+ 超過盒鬚圖左右邊之盒 $1.5 times (Q_3-Q_1)$ 到 $3 times (Q_3-Q_1)$ #highlight[距離內]之值可當作 #highlight[*可能（疑似）*] 之離群值。($Q_3-Q_1="IQR"$, #link(<四分位距>)[四分位距]。)
+ 超過盒鬚圖左右邊之盒 $3 times (Q_3-Q_1)$ 距離外之值可當作 #highlight[*非常可能（極端）*] 之離群值。

#question("Example 3.30", <ex3.30>, ("盒鬚圖",))[
  wip
][
  not done
]
...
== 枝葉圖（莖葉圖, stem-and-leaf diagrams）

- 保留原始資料的所有觀察值資訊，類似直方圖 (histogram)（定量資料）。
- 每筆資料由枝（莖，前頭的數字）和葉（最後一個數字）所快速組成。
...
#question("Example 3.28", <ex3.28>, ("枝葉圖",))[
  承 #link(<ex3.20>)[Example 3.20]，50 位學生性向測驗的成績如 @ex3.20-f1 所示。試著使用枝葉圖來呈現這 50 位學生性向測驗的成績。
][
  + 繪製初始枝葉圖（將數字根據單位塞入）：
    #table(
      columns: (1fr, 1fr),
