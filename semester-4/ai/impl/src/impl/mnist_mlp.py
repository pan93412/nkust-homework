...
model.add(Input((784,))) # 784维输入，512维输出
model.add(Dense(512))  # 全连接层，784维输入，512维输出
model.add(Activation('relu'))              # relu 激活函数
model.add(Dropout(0.25))                    # Dropout 舍弃部分因曾结点的权重，防止过拟合
for i in range(1,3):
    model.add(Dense(int(256/i)))                      # 由一个全连接层，512维输出
    model.add(Activation('relu'))              # relu 激活函数
    model.add(Dropout(0.25))                    # Dropout 舍弃部分因曾结点的权重，防止过拟合
