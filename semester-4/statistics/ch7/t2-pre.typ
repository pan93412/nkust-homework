...
#question(
  ...
  倘若今從班級中隨機抽出 $n=9$ 位女學生當成樣本，其樣本標準差 $s=2$，則這 9 位女同學之平均體重 $overline(x)$ 在某一數值 $k$ 以下的機率為0.99，試求此 $k$ 值為多少?($k$ 值四捨五入取到小數點後第
  2 位)
  ...
  *Solution*
  ...
  According to the t-distribution table, we can find that $t_(0.995)(8) = 2.896$.
  Therefore:
...
#question(
  ...
  *Q1*
  ...
  According to t-distribution table, we can find that $t_(alpha = 0.100)(k) = 1.440$.
  Therefore:
  ...
  *Q3*
  ...
  Since $t$ distribution is symmetric, we can calculate the probability of $P(T >= 1.44)$ and
  multiply it by 2.
  ...
  *Q4*
  ...
  According to t-distribution table, we can find that $t_(0.050)(k) = 1.895$ and $t_(0.010)(k) = 2.998$.
  Therefore:
...
#question(
  ...
  *Concept*
  ...
  Since it is a binomial distribution, we can use the *sample proportion* to solve
  this problem.
  ...
  Besides, the Z value of normal distribution in the possibility can be calculated
  from:
  ...
  *Solution*
  ...
  According to the _central limit theorem_, *when $n$ is large enough ($n p>5,
  n(1-p)>5$)*, the distribution of $hat(p)$ will be close to the normal
  distribution $N(mu=p, sigma^2=p(1-p)/n)$:
...
#question(
  ...
  *Q1*
  ...
  According to t-distribution table, we can find that when $r=1$, $t_(0.050)(k) = 6.314$.
  Therefore:
  ...
  *Q3*
  ...
  Since $t$ distribution is symmetric, we can calculate the probability of $P(T >= 6.314)$ and
  multiply it by 2.
  ...
  *Q4*
  ...
  According to t-distribution table, we can find that $t_(0.100)(k) = 1.866$ and $t_(0.025)(k) = 4.303$.
  Therefore:
...
#question(
  ...
  *Solution*
  ...
  We can find that *the variance of $hat(p)$ decreases as the sample size
  increases.* Therefore, the standard deviation of $hat(p)$ will decrease as the
...
#question(
  ...
  *Concept*
  ...
  Since the battery life is normally distributed, we can use the *sample mean* to
  solve this problem.
  ...
  *Solution*
  ...
  _Q1._ According to the _central limit theorem_, the distribution of the sample
  mean is $N(mu=400, sigma^2=10^2/25=4)$.
...
#question(
  ...
  某一常態母體，其母體平均數 $mu$ 未知，變異數 $sigma^2=13$，倘若從這個母體中隨機抽出 $n=20$ 的樣本，令樣本變異數為 $S^2$，試著求出機率：
  ...
  *Concept*
  ...
  The $E[X]$ of the population is unknown, so we should use the *Chi-Square Distribution* to solve this problem. Since $V(X)$ is known, we can calculate based on the variant of the population.

  The Chi-Square Distribution can be defined as follows:

  $
  S^2_mu &= (sum_(i=1)^n (X_i-mu)^2)/n \
  S^2_sigma &= (sum_(i=1)^n (X_i-overline(x))^2)/(n-1)
  $

  $
    X^2 = cases(
      n times S^2_mu / sigma^2 tilde chi^2(n) "if" mu "is known",
      (n-1) times S^2_sigma / sigma^2 tilde chi^2(n-1) "if" mu "is unknown"
    )
  $

  *Solution*

  The degree of freedom is $19$ since $mu$ is unknown.

  _Q1._ According to the Chi-Square Distribution table:

  $
  & P(S^2<18.61) \
  =& P((n-1) times S^2_sigma / sigma^2 < (19 times 18.61)/13) \
  =& P(chi^2 < 27.1992) \
  approx& 1-0.1 = 0.9
  $

  _Q2._ According to the Chi-Square Distribution table:

  $
  &P(S^2>20.62) \
  =& P(chi^2 < (19 times 20.62)/13) \
  =& P(chi^2 < 30.1369) \
  approx& 0.05
  $

  _Q3._ According to the Chi-Square Distribution table:

  $
  &P(6.924 < S^2 < 22.476) \
  =& P(S^2>6.924) - P(S^2>22.476) \
  =& P(chi^2 > (19 times 6.924)/13) - P(chi^2 > (19 times 22.476)/13) \
  =& P(chi^2 > 10.1197) - P(chi^2 > 32.8495) \
  approx& 0.95 - 0.025 = 0.925
  $
]
