...
#question("Question 21")[
  某公司生產之電腦鍵盤，要求此鍵盤長度(單位：英吋)的變異數不得超過0.0001，今隨機抽取20 個鍵盤，測量其長度，得到樣本變異數為0.0002，試在顯著水準 $alpha=0.05$ 下，分別以臨界值檢定法、 $chi^2$ 檢定法、p 值檢定法檢定鍵盤長度是否達成要求。
][
  *Step 1: 列出虛無和對立假說*

  $
  cases(
    H_0: sigma^2 <= 0.0001 "(達成要求)",
    H_1: sigma^2 > 0.0001 "(未達成要求)"
  )
  $
  ...
  *Step 2: 統計量分配、決策法則*
  ...
  $
  ((n-1) times S^2_mu)/sigma^2 tilde chi^2(19)
  $

  以 $alpha=0.05$ 為顯著水準，其拒絕域分別為：

  - 臨界值檢定法
    $
    alpha &= 0.05 \
      &= P(S^2_mu > c) \
      &= P(((n-1) times S^2_mu)/sigma^2 > ((n-1) times c)/sigma^2) \
    A &= ((n-1) times c)/sigma^2 = chi^2_(0.05)(19) \
    c &= chi^2_(0.05)(19) times sigma^2/(n-1) \
      &= 30.1435 times 0.0001/19 \
      &= 0.00015865
    $
    $
    R R = { S^2_mu | S^2_mu > 0.00015865 }
    $
  - $chi^2$ 檢定法
    $
    R R = { chi^2 | chi^2 > chi^2_(0.05)(19) }
    $
  - $p$ 值檢定法
    $
  ...
  *Step 3: 判斷是否在拒絕域*
  ...
  - 臨界值檢定法
    $
    S^2_mu = 0.0002 > 0.00015865
    $
    故屬於 $R R$，拒絕 $H_0$。
  - $chi^2$ 檢定法
    $
    chi^2 = 19 times 0.0002/0.0001 = 38 > 30.1435
    $
    故屬於 $R R$，拒絕 $H_0$。
  - $p$ 值檢定法 ⭐!
    $
    "p-value" &= P(S^2_mu > s = 0.0002) \
              &= P(chi > 19 times 0.0002 / 0.0001) \
              &= P(chi > 38) \
              &in (0.005, 0.01)
    $
    因為均小於 $0.05$，故屬於 $R R$，拒絕 $H_0$。

  *Step 4: 具體結論*

  在 $alpha=0.05$ 下，鍵盤長度未達成要求。
]
